...
with open('match_USopen_new.json', 'r') as file:
    m_US = json.load(file)

for anno in m_US:
    for partita in m_US[str(anno)]:
        US.insert_one(partita)
...
rank_US = db['rank US']
for anno in r_US:
    for giocatore in r_US[str(anno)]:
        giocatore['year']=anno
        rank_US.insert_one(giocatore)

for giocatore in rank_US.find():
    name=giocatore['Name'].replace("-", " ")
    name=name.title()
    rank_US.update_one({'_id': giocatore['_id']},
            {'$set': {'Name':name}})
...
client = MongoClient('mongodb://localhost:27017/')
db = client['Tennis'] 
rank_collection = db['rank US']
match_collection = db['US Open']
