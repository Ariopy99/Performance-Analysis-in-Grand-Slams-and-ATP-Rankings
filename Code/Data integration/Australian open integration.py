...
AUS = db['AUS Open']
import json
with open('match_AUS_new.json', 'r') as file:
    matches = json.load(file)

for anno in m_AUS:
    for partita in m_AUS[str(anno)]:
        AUS.insert_one(partita)

with open('rank_AUS_new.json', 'r') as file:
    r_AUS = json.load(file)

rank_AUS = db['rank AUS']
for anno in r_AUS:
    for giocatore in r_AUS[str(anno)]:
        giocatore['year']=anno
        rank_AUS.insert_one(giocatore)
    
for giocatore in rank_AUS.find():
    name=giocatore['Name'].replace("-", " ")
    name=name.title()
    rank_AUS.update_one({'_id': giocatore['_id']},
            {'$set': {'Name':name}})
...
client = MongoClient('mongodb://localhost:27017/')
db = client['Tennis'] 
rank_collection = db['rank AUS']
match_collection = db['AUS Open']
